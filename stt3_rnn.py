...
def bind_model(model, parser):
    ...
    def infer(test_path, **kwparser):
        ...
        test_dataset = CustomDataset(test_file_list,transform=valid_audio_transforms, mode="test")
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
       #stt3 train 파일 생성 수정 필요  
        ...
        train_label = [clean(sen) for sen in splited_train_label]
        ...
        train_dataset = CustomDataset(splited_train_file, train_tokens, transform=train_audio_transforms, max_size=None, min_size=0)
        valid_dataset = CustomDataset(val_file_list, valid_tokens, transform=valid_audio_transforms, max_size=None, min_size=0)
        ...
        criterion = nn.CTCLoss(blank=tokenizer.txt2idx["<pad>"]).to(device)
        ...
        for epoch in range(int(args.reload_from / n_iters) + 1, args.total_epoch + 1):
            ...
            for batch in train_dataloader:
                ...
                speech = batch["speech"].to(device)
                text = batch["labels"].to(device)
                in_length = batch["in_length"]
                out_length = batch["out_length"]
                
                output = model(speech)
                output = F.log_softmax(output, dim=2)
                output = output.transpose(0, 1) 
                
                loss = criterion(output, text, in_length ,out_length)
